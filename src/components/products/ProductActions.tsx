...
export const ProductActions: React.FC<ProductActionsProps> = ({
  ...
  // Dynamic button color matching reference image exactly
  ...
  if (brandLower.includes('amaron')) {
    ...
    enquireBgClass = 'bg-[#ef4444] hover:bg-[#dc2626] text-white shadow-[0_4px_14px_rgba(239,68,68,0.35)]';
  ...
  return (
    <div id={`actions-${product.id}`} className={`grid grid-cols-2 gap-2.5 w-full ${className}`}>
      ...
      <button
        ...
        className="flex items-center justify-center gap-1.5 py-2.5 px-3 rounded-xl bg-[#0e131d]/90 hover:bg-[#151c2a] text-white font-semibold text-xs border border-[#1e2638] hover:border-neutral-600 transition-all duration-200 cursor-pointer shadow-sm hover:scale-[1.02] active:scale-[0.98]"
      >
        <span>Details</span>
      ...
      <button
        ...
        className={`flex items-center justify-center gap-1.5 py-2.5 px-3 rounded-xl font-bold text-xs shadow-md transition-all duration-200 cursor-pointer hover:scale-[1.02] active:scale-[0.98] ${enquireBgClass}`}
      >
